...
int main(int argc, char** argv) {
	...
	while(1){
		...
		switch(opt)
		{
			...
			case 'h':
				std::cout << "\n******************************************************************************************************************************* \n\n\t";
				std::cout << "\n This script is used in AI Engine Compiler to generate CDO binaries and record statically allocated resources. Files generated by this script are:\n\n"
						"\t\t\t >> aie_cdo_init.bin            : Initial Configurations CDO. Stream Switch, DMA, SHIM etc...\n"
						"\t\t\t >> aie_cdo_enable.bin          : Core Enable CDO\n"
						"\t\t\t >> aie_cdo_debug.bin           : Core Debug Halt CDO \n"
						"\t\t\t >> aie_cdo_reset.bin           : Partition Reset CDO \n"
						"\t\t\t >> aie_cdo_mem_clear.bin       : Partition DM/PM clearing CDO\n"
						"\t\t\t >> aie_cdo_error_handling.bin  : Error Handling CDO \n"
						"\t\t\t >> aie_cdo_clock_gating.bin    : Clock Gating CDO \n"
						"\t\t\t >> aie_cdo_elfs.bin            : AIE ELF packed into CDO (Default ECC-scrubbing enabled)\n"						"\t\t\t >> aie_resources.bin           : Statically allocated resource file";
				std::cout << "\n******************************************************************************************************************************* \n\n\t";
				std::cout << "\nAccepted options are :\n\n\t"
				"1) --help or -h: Displays list of options accepted by cdo executable.(Usage: --help) \n\n\t"
				"2) --aximm-dump or -a: Generates AXI-MM Dump helpful for debugging.(Usage: --aximm-dump)\n\n\t"
				"3) --big-endian-cdo or -b: Generates AIE CDO in big endian format, default is little-endian (Usage: --big-endian-cdo) \n\n  ";
				return EXIT_SUCCESS;
			case 1:
			case 'a':
				AXIdebug = true;
			...
			case '?':
				...
				break;
			default:
				std::cout << "getopt returned char code which is not handled, returned code:" << opt << std::endl;
				return EXIT_FAILURE;
			}
		}
	if (optind < argc) {
