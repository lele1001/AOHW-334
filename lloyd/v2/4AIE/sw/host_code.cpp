...
#define arg_sink_from_aie_output 4
#define arg_sink_from_aie_size 5
...
std::vector<Cluster> k_means(const std::vector<float> &input, int32_t num_clusters, int32_t num_points)
{
    ...
    // Updates the cluster coordinates
    for (size_t i = 0; i < num_clusters; i++)
    {
    ...
    for (size_t j = 0; j < clusters_vec.size(); j++)
    {
        for (size_t pow = 9; pow < max_pow + 1; pow += step)
        {
            ...
            for (size_t i = 0; i < num_clusters; i++)
            {
                ...
                // Copy the cluster coordinates to the hw_buffer as integers pointing to floats
            ...
            for (size_t i = 0; i < num_points; i++)
            {
                ...
                // Copy the point coordinates to the input buffer as integers pointing to float
