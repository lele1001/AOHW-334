...
entity setup_aie_control_s_axi is
generic (
...
end entity setup_aie_control_s_axi;
...
architecture behave of setup_aie_control_s_axi is
    ...
    constant ADDR_NUM_CLUSTERS_DATA_0 : INTEGER := 16#10#;
    constant ADDR_NUM_CLUSTERS_CTRL   : INTEGER := 16#14#;
    constant ADDR_NUM_POINTS_DATA_0   : INTEGER := 16#18#;
    constant ADDR_NUM_POINTS_CTRL     : INTEGER := 16#1c#;
    constant ADDR_INPUT_R_DATA_0      : INTEGER := 16#20#;
    constant ADDR_INPUT_R_DATA_1      : INTEGER := 16#24#;
    constant ADDR_INPUT_R_CTRL        : INTEGER := 16#28#;
    ...
    signal int_num_clusters    : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUM_CLUSTERS_DATA_0 =>
                        rdata_data <= RESIZE(int_num_clusters(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    num_clusters         <= STD_LOGIC_VECTOR(int_num_clusters);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUM_CLUSTERS_DATA_0) then
                    int_num_clusters(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_num_clusters(31 downto 0));
                end if;
            end if;
        end if;
    end process;
