...
std::vector<Cluster> k_means(const std::vector<float> &input, int32_t num_clusters, int32_t num_points)
{
    ...
    for (size_t i = 0; i < num_points; i++)
    {
        ...
        // Finds the index of the nearest cluster
    ...
    for (size_t j = 0; j < clusters_vec.size(); j++)
    {
        for (size_t pow = 9; pow < max_pow + 1; pow += step)
        {
            ...
            for (size_t i = 0; i < num_clusters; i++)
            {
                ...
                // Copy the cluster coordinates to the input buffer as integers pointing to floats
            ...
            for (size_t i = 0; i < num_points; i++)
            {
                ...
                // Copy the point coordinates to the input buffer as integers pointing to floats
                int32_t val_x = *reinterpret_cast<int32_t *>(&input_buffer_sw[i * 2 + 0]);
                int32_t val_y = *reinterpret_cast<int32_t *>(&input_buffer_sw[i * 2 + 1]);
